...
This is the source code for my webpage which is hosted at https://deej-ai.online/. The backend now uses JEPA music embeddings generated by https://github.com/teticio/music-jepa. Some older TensorFlow/audio-similarity artifacts are still used for the optional MP3 similarity endpoint.
...
* FastAPI backend server that uses JEPA embeddings for playlist generation, serves web content, manages the database (SQL, SQLite, etc.) and takes care of the Spotify authentication flow.
...
Playlist generation defaults to `EMBEDDINGS_MODEL=jepa` and expects local, memory-mapped JEPA artifacts derived from https://github.com/teticio/music-jepa. The conversion script is:

```
make regenerate_jepa_embeddings
```
...
* `../jepa/embeddings/embeddings.npy` by default, produced by `music-jepa`. You can override this with `--source`.
* `model/tracktovec.p`, used as the playlist metadata/behaviour channel.
* `model/spotifytovec.p`, used for the optional audio-similarity channel.

Outputs:

* `model/jepa_ids.npy`
* `model/jepa_vectors.npy`
* `model/tracktovec_ids.npy`
* `model/tracktovec_vectors.npy`
* `model/spotifytovec_ids.npy`
* `model/spotifytovec_vectors.npy`
* `model/jepa_tracktovec_vectors.npy`
* `model/jepa_spotifytovec_vectors.npy`

The `jepa_*_vectors.npy` companion files are pre-aligned to `jepa_ids.npy` so the backend can load the matrices with NumPy memory mapping instead of rebuilding large Python dictionaries at startup.
